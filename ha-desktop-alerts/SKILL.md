...
- **默认只认目标事件,别把触发词写宽**:用户要"门铃弹窗"就别把`有人/经过/报警`也放进去 ——
  否则后院的摄像头、喂食器消息也会弹屏(实测被用户点名抱怨"画面不对"+"只管门铃")。
  做法:先按 `device_id`/`model` 锁定设备,再按 `event` 码锁定事件(门铃=`7.1006`),
  其余消息**只写日志不弹**;留一个 `DOORBELL_ONLY=0` 开关给需要放宽的场合。
...
- ⚠️ **PowerShell 5.1 读 `.ps1` 默认按 ANSI,脚本里写中文会变乱码**:曾把按钮"忽略"变成
  `蹇界暐` 而用户反馈"按钮上那3个字不认识"。铁律:**投递脚本(`.ps1`/`.vbs`)只写 ASCII**,
  所有中文从 UTF-8 的 JSON/文本文件用 `ReadAllText(path, UTF8)` 读;实在要写死就给文件加
