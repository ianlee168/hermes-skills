...
## 每盏灯独立光晕贴片的正解(2026-09-13 实做)

需求：“按哪个开关，哪个房间亮光晕”，且要跟图标位置对得上。

❌ **不要用“开灯图 − 关灯图”的差值做贴片**：差值里混着墙体/门等脏像素，
  叠上去就是一块带硬边的贴纸（用户原话：“太乱”）。

✅ **正确做法——拿底图自己的像素提亮暖化**，肉眼看就是灯光洒在地上：

```python
crop = base[y0:y0+ph, x0:x0+pw]                       # 底图自身的裁剪
lit  = clip(crop*warm*0.72 + crop*0.28 + add, 0, 255)  # 暖化+提亮
# warm=[1.0,0.93,0.72](暖白) [1.1,1.09,1.05](白) [1.05,1.09,1.14](冷白); add=[26,17,-4] 左右
 u=(xx-pw/2)/(pw/2); v=(yy-ph/2)/(ph/2)
a = clip(1-sqrt(u*u+v*v), 0, 1)**1.3 * 0.85              # 椭圆径向羽化
save RGBA(crop, a*255): edge 天然为 0 → 无缝
```

要点：
- 贴片像素 = 底图同区域像素 → **永不会错位/露边**，只需控制中心与半径
- 尺寸按房间给：大灯 18%×20%，普通射灯 9–12%×12–14%（以 1024×576 底图为基准）
- 中心直接用**该灯图标的位置**（用户自己拖过的那份）——用户说“跟着图标改”就是指这个
- 每盏灯一个 `figtype: conditional`(state=on) + 内嵌 `type: image`，`title` 写清楚是哪盏灯，
  以后改位置按 title 匹配最好找
