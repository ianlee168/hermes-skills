...
## 任务分工（陛下 2026-09-22 定，两边共守）

| 任务类型 | 归谁 | 备注 |
|---|---|---|
| **要 GPU，或要碰本机资源**（Windows 桌面弹窗、Chrome 已登录态、`D:`/`H:` 素材与模型、微信 iLink、USB/摄像头、本机进程） | **110妹 / 西宫妹妹（50.110）** | 在线窗口仅约 **10:30–02:00**（凌晨到中午基本关机）；长任务必须能中断续跑 |
| **要 24/7、深夜/清晨定点触发，或守护类**（看门狗/巡检/对外服务） | **161姐 / 东宫姐姐（50.161）** | **24/7 常开**；⚠ 4 核 / 7.2G（实测可用 ~4.6G）/ 无独显无 CUDA —— 别派重活（量化见下） |
| 两者都不要（纯网络 / 纯运维琐活） | 161姐（一直在线上，秒级响应） | —— |
| 同上，但数据在某台的本地盘 | **数据所在那台** | 省得跨机搬 GB 级文件；能"就地算、只搬结论"就别搬原件 |
| 跨机迁移 / 对账 | **谁在线谁做，发起端优先** | —— |

> **核心区别就两条：东宫 = 24/7 常开但无 GPU；西宫 = 有 GPU、能碰陛下本机资源。**
> 表里的时间窗口是**参考值，不是排期铁律**（陛下原话：「不用那么教条，到几点几点，就是 24 小时在线和 GPU 运算的区别」）。
...
**不做自动睡眠（陛下 2026-09-22 定）**：唤醒后机器就照常开着，陛下按自己的作息关机；东宫姐姐**不需要也不要**代为关机/睡眠。这条是为了避免"她叫醒了却没人管"的误会 —— 叫醒后不用管，剩下的交给陛下。
...
- **2026-09-22 · 161姐**：清理了 Unraid 侧
  `/boot/config/plugins/user.scripts/scripts.bak-20260909/Gemini_HA_Control/script` 里的**明文 Home Assistant 长期令牌**
  （ianlee168 授权；undo = 同目录 `script.bak-20260922`，权限 600 保持；改后本地 `py_compile` 通过）。
  该脚本是 2026-02-14 的备份 user-script，未在启用目录、无 cron 引用。
  → 请 **50.1 侧**核对 `smart-home` / HA 相关脚本是否还有同类明文（HA 令牌 = 全屋控制权）；
  统一规矩：**脚本与文档一律不写明文令牌**，用
  `HA_TOKEN=$(grep -m1 '^HA_TOKEN=' ~/.hermes/.env | cut -d= -f2-)`，权威副本见脑页
  `concepts/home-assistant-token-2026-09-22`。同一批还清了 50.161 上 3 个同名明文文件 + 1 处 history。
