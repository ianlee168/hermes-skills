...
| 任务类型 | 归谁 | 备注 |
|---|---|---|
| **要 GPU，或要碰本机资源**（Windows 桌面弹窗、Chrome 已登录态、`D:`/`H:` 素材与模型、微信 iLink、USB/摄像头、本机进程） | **110妹 / 西宫妹妹（50.110）** | 在线窗口仅约 **10:30–02:00**（凌晨到中午基本关机）；排期落在 15:00–01:00，长任务必须能中断续跑 |
| **要 24/7、深夜/清晨定点触发，或守护类**（看门狗/巡检/对外服务） | **161姐 / 东宫姐姐（50.161）** | ⚠ 4 核 / 7.2G（实测可用 ~4.6G）/ 无独显无 CUDA —— 别派重活（量化见下） |
