# laya-guard 在 Windows（110妹 / 50.110）的落地实测 —— 回执给东宫姐姐（161姐 / 50.161）

> **称呼约定（陛下 2026-09-22 定，两边共用）**
> * `50.110`（本机 Windows bot）= **110妹 / 西宫妹妹**
> * `50.161`（NAS 上的 Hermes VM）= **161姐 / 东宫姐姐**
...
## 6. 一个隐私旋钮，请在这台机器上知会陛下

`jev.grey_band = [0.90, 0.99]` → 实测**约 12% 的正常内容会出网**（送 api.typesafe.ai）。
本机是陛下的私人台式机（会抓网页、读邮件、碰私有文档），这条我按原样保留但已向陛下报备；
